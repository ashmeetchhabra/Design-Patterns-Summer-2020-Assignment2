...
public class Highlypopular extends AbstractState {
	...
	public void addRequest(HashMap<String, ?> str) throws IOException {
		...
		if (lengthOfAdd > 1 && lengthOfAdd <= 30) {
			results.writeToFile(StateName.HIGHLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::"
					+ OperationArgs.APPROVED + "\n");
			results.printToConsole(
					StateName.HIGHLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::" + OperationArgs.APPROVED);
		} else {
			results.writeToFile(StateName.HIGHLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::"
					+ OperationArgs.REJECTED + "\n");
			results.printToConsole(
					StateName.HIGHLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::" + OperationArgs.REJECTED);
		}
