...
public class MildlyPopular extends AbstractState {
	...
	public void addRequest(HashMap<String, ?> str) throws IOException {
		...
		if (lengthOfAdd > 1 && lengthOfAdd <= 20) {
			results.writeToFile(StateName.MILDLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::"
					+ OperationArgs.APPROVED + "\n");
			results.printToConsole(
					StateName.MILDLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::" + OperationArgs.APPROVED);
		} else {
			results.writeToFile(StateName.MILDLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::"
					+ OperationArgs.REJECTED + "\n");
			results.printToConsole(
					StateName.MILDLY_POPULAR.name() + OperationArgs.__AD_REQUEST + "::" + OperationArgs.REJECTED);
		}
